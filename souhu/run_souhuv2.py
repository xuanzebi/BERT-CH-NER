...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(self._read_data(os.path.join(data_dir, "coreEntityEmotion_train.txt"), mode='train'),
                                    "train")
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        total_loss, logits, trans, pred_ids = create_model2(
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, pred_ids):
                precision = tf_metrics.precision(label_ids, pred_ids, 8, [1, 2, 3, 4], average="macro")
                recall = tf_metrics.recall(label_ids, pred_ids, 8, [1, 2, 3, 4], average="macro")
                f = tf_metrics.f1(label_ids, pred_ids, 8, [1, 2, 3, 4], average="macro")
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        fw = open(os.path.join(FLAGS.output_dir, "test_predictionv2.txt"), 'a+', encoding='utf-8')
        print('***********正在写入**************')
        for i in result:
