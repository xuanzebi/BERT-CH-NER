            ...
            with open(input_file, encoding='utf-8') as f:
                ...
                for l in tqdm(f):
                    ...
                    for seq in sentences:
                        ...
                        trn_data.append((news_id, w, l))
                import random
                random.shuffle(trn_data)
                index = int(0.8 * len(trn_data))
                if mode == 'train':
                    return trn_data[:index]
                if mode == 'dev':
                    return trn_data[index:]
        if mode == 'test':
            with open(input_file, encoding='utf-8') as f:
...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "coreEntityEmotion_train.txt"), mode='train'),
            "train")

    def get_dev_examples(self, data_dir):
        return self._create_example(self._read_data(os.path.join(data_dir, "coreEntityEmotion_train.txt"), mode='dev'),
                                    "dev")

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "coreEntityEmotion_test_stage1.txt"), mode='test'), "test"
        )

    def get_labels(self):
        return ["PAD", "B", "I", "E", "S", "O", "[CLS]", "[SEP]"]
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            labels = tokenization.convert_to_unicode(line[2])
...
#         import json
#         res = []
#         print('len(result),len(newsid_examples):')
#         print(len(result), len(newsid_examples))
#         assert len(result) == len(newsid_examples)
#         for i in range(len(result)):
#             output = " ".join(id2label[id] for id in result[i] if id != 0 or id != 6 or id != 7)
#             output = output.split(' ')
#             newsid = newsid_examples[i][0]
#             content = newsid_examples[i][1]
...
#             while j < len(output):
#                 if output[j] == 'S':
#                     entity.append(content[j])
#                     j += 1
#                 elif output[j] == 'B':
#                     while k < len(output):
#                         if output[k] == 'E':
#                             entity.append(content[j:k + 1])
#                             j = k + 1
#                             break
#                         else:
#                             k += 1
#                 else:
#                     j += 1
#             res.append({'newsid': newsid,
#                         'content': content,
#                         'pred_label': output,
#                         'entity': entity})
#         with codecs.open(os.path.join(FLAGS.output_dir, "test_pred.json"), 'w', encoding='utf-8') as f:
